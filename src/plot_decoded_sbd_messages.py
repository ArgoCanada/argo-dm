...
wmo = 4902671
flist = list(Path('../sbd/adopted-{}'.format(wmo)).glob('Ascent*.csv'))
print(flist)
fn = flist[0]
df = pd.read_csv(fn, sep=';', encoding='unicode-escape')
pres = df['CTD - Pressure (dbar)'].to_numpy()
temp = df['CTD - Temperature (deg C)'].to_numpy()
...
fig.set_size_inches(w*3/3, h)

fig.savefig(Path('../figures/{}_initial_profile.png'.format(wmo)), bbox_inches='tight', dpi=250)
plt.close(fig)