...
df['nsbd_rbr'] = np.ceil(df.npacket_rbr/3)
df['data_kb'] = df.nsbd*300/1000
df['data_bgc_kb'] = df.nsbd_bgc*300/1000
df['data_rbr_kb'] = df.nsbd_rbr*300/1000
...
ax.plot(df.npts, df.data_rbr_kb, 's', color='navy', label='NKE RBR')
...
rf = np.poly1d(np.polyfit(df.data_rbr_kb, df.npts, 1))
...
for i,d in enumerate(dplans):
    ...
    # ax.axvline(rf(d/3), color=pal[2+i], linestyle='-.', label=None)
    print(cf(d/3), bf(d/3), rf(d/3))
...
ax.plot([288], [5.4], '*', color='grey', markersize=10, label='New High-res MEDS DO Float')
...
ax.minorticks_on()
ax.grid()
fig.savefig(Path('../figures/approximate_data_usage_20210427.png'), bbox_inches='tight', dpi=350)
plt.close(fig)