...
ax.plot(meds_core_pts, meds_core_data, 'o', color='orange', label='MEDS Core')
ax.plot(coriolis_core_pts, coriolis_core_data, 'mo', label='Coriolis Core')
ax.plot(meds_bgc_pts, meds_bgc_data, '^', color='orange', label='MEDS BGC')

ax.set_xlabel('Average Number of Points per Profile')
ax.set_ylabel('File Size per Profile (kB)')
ax.legend(loc=2)
ax.set_xlim((-5, 1050))
ax.set_ylim((0,20))
plt.show()
